...
import com.example.rollingChunks.model.ChunkPart;
...
import com.example.rollingChunks.repository.FileVersionRepository;
import com.example.rollingChunks.repository.UploadSessionRepository;
...
public class ChunkedFileService {
    ...
    public InitUploadResponse initUpload(InitUploadRequest req) {
        ...
        // We return at most one MissingPart per unique hash since uploading the same content-hash
        // chunk multiple times is redundant.
        for (InitPart part : req.parts()) {
            String hash = part.hash();

            if (presignedPutByHash.containsKey(hash) || session.hasReceived(hash)) {
                continue;
            }
    ...
    private void finalizeAvailable(FileMetadata file, FileVersion version, UploadSession session) {
        // Idempotency: this can be invoked via async S3 notification and also via the
        // client calling /complete. We only want one durable change event.
        boolean alreadyAvailable = file.getStatus() == FileStatus.AVAILABLE
                && version.getStatus() == FileStatus.AVAILABLE
                && version.getId().equals(file.getCurrentVersionId());
        ...
        changeFeedService.recordFileAvailable(file, version);
    }
