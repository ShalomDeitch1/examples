...
public class ReadDao {
    ...
    @Transactional(readOnly = true, transactionManager = "replicaTransactionManager")
    public List<Models.Warehouse> findWarehousesByIds(List<UUID> warehouseIds) {
        if (warehouseIds.isEmpty()) {
            return List.of();
        }

        String placeholders = warehouseIds.stream().map(id -> "?").reduce((a, b) -> a + "," + b).orElse("?");
        String sql = "SELECT warehouse_id, name, latitude, longitude FROM warehouses WHERE warehouse_id IN (" + placeholders + ")";
