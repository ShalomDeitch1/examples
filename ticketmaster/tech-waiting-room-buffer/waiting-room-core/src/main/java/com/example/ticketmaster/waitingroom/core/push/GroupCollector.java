 ...
 * - In push-mode integrations, the broker invokes a listener whenever a message arrives.
 * - The demo’s processing model is tick/batch-based; this buffer bridges push delivery into scheduled batch processing.
 *
 * Real system notes:
 * - You’d usually avoid an in-process buffer for durability/scale. Options include: processing directly in the listener,
 *   using a durable internal queue/stream, or using partitioned work queues with consumer lag metrics.
 *
 * How it fits this example flow:
 * - Listener enqueues request IDs; the scheduled processor drains them up to N per tick.
 */
