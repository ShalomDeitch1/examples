 ...
 * - Exposes the shared HTTP API for the Kafka pull-mode demo (enqueue + observability).
 *
 * Real system notes:
 * - Public APIs are typically authenticated and protected by rate limiting; internal state is stored durably and queried via dedicated endpoints/metrics.
 *
 * How it fits this example flow:
 * - POST creates a WAITING request in {@code RequestStore} then publishes the request ID to Kafka.
